...
const CodeSamplesTable = ({ category }) => {
  // Filter codeSamples by html_id if category prop is provided
  const filteredSamples = category
    ? codeSamples.filter((group) => group.html_id === category)
    : codeSamples
            ...
            <React.Fragment key={group.category}>
              ...
              {group.links.map((link) => (
                <tr key={link.url}>
                  ...
                  <td dangerouslySetInnerHTML={{ __html: link.description }} />
                </tr>
              ))}
            </React.Fragment>
          ))}
